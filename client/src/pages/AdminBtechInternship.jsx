...
import { Search, Eye, Trash2, Calendar, Filter, RefreshCw, FileSpreadsheet, MapPin, Briefcase, Phone, Globe, Plus, Users, Edit2, GraduationCap as GradIcon } from 'lucide-react';
...
const AdminBtechInternship = () => {
  ...
  return (
    <AdminLayout title={<>Andhra Pradesh- <span className="text-emerald-500 not-italic uppercase">Industrial Training - APSBTET</span></>} subtitle="Manage specialized industrial training applications">
                ...
                <tr key={app._id} className="hover:bg-emerald-50/30 transition-all group/row">
                  ...
                  <td className="px-8 py-6">
                    <div className="flex justify-end gap-3">
                      <Link to={`/admin/btech-internship/${app._id}`} className="p-3 bg-emerald-500/5 text-emerald-600 rounded-xl hover:bg-emerald-600 hover:text-white transition-all shadow-sm" title="View Details"><Eye size={18} /></Link>
                      <button onClick={() => navigate(`/admin/btech-internship/edit/${app._id}`)} className="p-3 bg-primary/5 text-primary rounded-xl hover:bg-primary hover:text-white transition-all shadow-sm" title="Edit Details"><Edit2 size={18} /></button>
