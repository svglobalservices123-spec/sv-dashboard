...
import { submitDiplomaInternship, getDiplomaInternshipDetails, updateDiplomaInternship } from '../utils/api';
...
const AdminAddDiplomaInternship = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) {
        // For editing, we might just send JSON if no new files are selected
        // But to keep it consistent and handle potential file updates, we use FormData
        ...
        await updateDiplomaInternship(id, data);
        toast.success('Diploma Internship record updated!');
      } else {
        const data = new FormData();
  ...
  return (
    <AdminLayout title={isEdit ? "Update Application" : "Manual Enrollment"} subtitle={isEdit ? `Editing details for ${formData.studentFullName}` : "Add diploma internship application manually"}>
