...
const AdminAddDiplomaInternship = () => {
  ...
  const InputField = ({ label, name, type = "text", placeholder, required = true, options = null }) => (
    <div className="space-y-2">
      ...
      {options ? (
        <select name={name} value={formData[name]} onChange={handleInputChange} required={required} className="input-field py-4 bg-muted/30 border-transparent focus:bg-white focus:border-primary cursor-pointer">
          <option value="">Select {label}</option>
          {options.map(opt => <option key={opt.value || opt} value={opt.value || opt}>{opt.label || opt}</option>)}
        </select>
      ) : (
        <input type={type} name={name} value={formData[name]} onChange={handleInputChange} required={required} className="input-field py-4 bg-muted/30 border-transparent focus:bg-white focus:border-primary" placeholder={placeholder} />
      )}
    </div>
  );

  return (
    <AdminLayout title="Manual <span className='text-secondary uppercase not-italic'>Enrollment</span>" subtitle="Add diploma internship application manually">
        ...
        <div className="bg-white rounded-[2.5rem] shadow-2xl shadow-gray-200/50 border border-gray-100 p-8 md:p-12 space-y-12">
          ...
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <InputField label="City" name="city" placeholder="e.g. Hyderabad" />
          ...
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <InputField label="Academic GPA" name="gpa" placeholder="e.g. 8.5" />
            </div>
          </section>
