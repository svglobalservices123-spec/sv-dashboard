...
import { getBtechInternshipDetails, deleteBtechInternship } from '../utils/api';
...
const BtechInternshipDetails = () => {
  ...
  const DetailItem = ({ label, value, icon, isDark = false }) => (
    <div className="group">
      <p className={`text-[10px] font-black uppercase tracking-widest ml-1 mb-1 group-hover:text-emerald-600 transition-colors flex items-center gap-2 ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>
        {icon} {label}
      </p>
  ...
  const DocumentCard = ({ label, doc }) => (
    <div className="bg-muted/50 p-6 rounded-3xl border border-gray-100 hover:border-emerald-300 transition-all group flex items-center justify-between">
      ...
      <a href={doc?.url} target="_blank" rel="noopener noreferrer" className="p-3 bg-emerald-600 text-white rounded-xl hover:scale-110 active:scale-95 transition-all shadow-lg shadow-emerald-500/20">
  ...
  return (
    <AdminLayout title={<>Btech Intern <span className="text-emerald-500 not-italic">Profile</span></>} subtitle="Detailed administrative view of degree internship application">
      <div className="space-y-10 pb-20">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-6">
          <button onClick={() => navigate('/admin/btech-internship')} className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-gray-400 hover:text-emerald-600 transition-all">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <DetailSection title="Academic Profile" icon={<GraduationCap size={16} />}>
            ...
            <DetailItem label="Degree Percentage" value={app.degreePercentage} icon={<CheckCircle size={10} />} />
          ...
          <div className="space-y-8">
            <div className="bg-dark rounded-[2.5rem] p-10 text-white shadow-2xl shadow-emerald-900/20">
              <h3 className="text-xs font-black uppercase tracking-[0.3em] text-emerald-400 flex items-center gap-3 mb-8">
                <span className="w-8 h-8 rounded-lg bg-emerald-500/20 flex items-center justify-center"><Briefcase size={16} /></span>
                Training Parameters
              </h3>
              <div className="space-y-6">
                <DetailItem label="Mode of Training" value={app.trainingMode} icon={<Briefcase size={10} className="text-emerald-400" />} isDark={true} />
                <DetailItem label="Target Course" value={app.course} icon={<GraduationCap size={10} className="text-emerald-400" />} isDark={true} />
                <DetailItem label="Company Name" value={app.companyName} icon={<Briefcase size={10} className="text-emerald-400" />} isDark={true} />
              </div>
...
export default BtechInternshipDetails;
