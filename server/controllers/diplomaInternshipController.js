...
exports.updateApplication = async (req, res, next) => {
  try {
    ...
    for (const key of docKeys) {
      if (files[key] && files[key][0]) {
        const file = files[key][0];
        const fileName = `${updateData.studentFullName || 'Updated'}_${key}_${Date.now()}${path.extname(file.originalname)}`;
    ...
    const application = await DiplomaInternship.findByIdAndUpdate(id, updateData, { new: true });
